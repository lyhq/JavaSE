package com.lyhq.design.patterns.Composite.Standard.Clarity;

/**
 * 组合模式 （透明的组合模式，不推荐使用）
 * 透明模式是把用来组合使用的方法放到抽象中，比如add()、remove()以及getChildren等方法（），
 * 不管叶子对象不是树枝对象都有相同的结构，通过判断是getChildren的返回值确认是叶子节点 不是树枝节点，
 * 如果处理不当会在运行期出现错误，所以建议用上述例子中的安全模式。
 * 
 * 1.优点：
